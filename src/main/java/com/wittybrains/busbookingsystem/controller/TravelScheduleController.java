...
public class TravelScheduleController {
  ...
  @PutMapping("/{id}")
  public ResponseEntity<TravelSchedule> updateSchedule(@PathVariable Long id, @RequestBody TravelScheduleDTO updatedSchedule) {
	  Optional<TravelSchedule> existingScheduleOptional = scheduleRepository.findById(id);
	    if (existingScheduleOptional.isPresent()) {
	        ...
	        // Check if the busId exists in the database
	        Optional<Bus> busOptional = busRepository.findById(updatedSchedule.getBus().getId());
